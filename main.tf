...
terraform {
  ...
  backend "s3" {
    bucket = "tfstate-bucket-b128f891" # 미리 생성한 s3 버킷의 이름
    key = "test01/terraform.tfstate"  # /lecture/하위에 만들어 지도록
    region = "ap-northeast-2"
    dynamodb_table = "terraform-lock-test01"  # 미리 준비된 dynamodb 테이블의 이름을 명시하면 lock 상태가 자동으로 관리됨
